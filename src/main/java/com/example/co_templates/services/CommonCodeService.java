...
public class CommonCodeService {
    ...
    public Object selectTotal(Map dataMap) { 
        String sqlMapId = "CommonCode.selectTotal"; // 전체 카운트(pagination)
        Object result = shareDao.getOne(sqlMapId, dataMap);
        return result;
    }

    public Map selectSearchWithPagination(Map dataMap) { // list 정보와 pagination 정보를 넘기는 메서드
        // 페이지 형성 위한 계산
        int totalCount = (int) this.selectTotal(dataMap);

        int currentPage = 1;//페이지로 숫자가 안들어올수도 있어서 넣는것, 1page 라는 의미
        if(dataMap.get("currentPage") != null) {
            currentPage = Integer.parseInt((String)dataMap.get("currentPage"));    // from client in param
        }
        Paginations paginations = new Paginations(totalCount, currentPage); // total 값과 pagination 값
